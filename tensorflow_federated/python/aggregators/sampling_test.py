...
# Type for the random seed used in sampling is a pair of int64 scalars.
SEED_TYPE = (computation_types.TensorType(tf.int64),
             computation_types.TensorType(tf.int64))
TEST_SEED = 42
# Type for the random values assigned to samples.
RANDOM_VALUE_TYPE = computation_types.TensorType(tf.int32, [None])


# DO NOT SUBMIT: this is a terrible workaround to get our python container...
...
class BuildReservoirTypeTest(test_case.TestCase):
  ...
  def test_fails_non_tensor_or_struct_type(self):
...
class BuildInitialSampleReservoirTest(test_case.TestCase):
  ...
  def test_scalar(self):
    ...
    self.assertAllEqual(
        initial_reservoir,
        collections.OrderedDict(
            random_seed=(TEST_SEED, TEST_SEED),
            random_values=tf.zeros([0], tf.int32),
            samples=tf.zeros([0], dtype=tf.int32)))
    # Build without a seed, using the timestamp.
    initial_reservoir = sampling._build_initial_sample_reservoir(
        TensorType(tf.int32))
    ...
    self.assertGreater(initial_reservoir['random_seed'][0], 1**32)
...
class BuildSampleValueComputationTest(test_case.TestCase):
  ...
  def test_scalar(self):
    ...
    sample_computation = sampling._build_sample_value(
...
class BuildMergeSamplesTest(test_case.TestCase):
  ...
  def test_scalar(self):
    example_type = TensorType(tf.int32)
    merge_computation = sampling._build_merge_samples(
...
class BuildFinalizeSampleTest(test_case.TestCase):
  ...
  def test_scalar(self):
    ...
    finalize_computation = sampling._build_finalize_sample(example_type)
    reservoir_type = sampling._build_reservoir_type(example_type)
    expected_type = FunctionType(
